    ...
    static function ajax_run_tool()
    {
        ...
        if ($tool == '404_logs') {
            ...
            if($group != 'ip' && $group != 'url'){
                wp_send_json_error('Unknown group');
            }

            $wpdb->query($wpdb->prepare("DELETE FROM " . $table . " WHERE " . $group . " = %s", $value));
            ...
            self::get_logs_group();
        } else if ($tool == 'reset_hits') {
            $wpdb->query("UPDATE " . $wpdb->wf301_redirect_rules . " SET last_count='0'");
            wp_send_json_success();
    ...
    static function get_logs_group()
    {
        ...
        foreach ($rResult as $aRow) {
            ...
            foreach ($groupedvals as $groupedval) {
                if($group == 'ip'){
                    $groupedvals_array[] = '<a href="' . WF301_functions::get_formatted_full_url(stripslashes($groupedval->val)) . '" target="_blank">' . htmlspecialchars(stripslashes($groupedval->val)) . '</a>';
                } else {
            ...
            $row[] = '<div data-delete-log="' . $log . '" data-delete-group="' . $group . '" data-delete-group-value="' . ($group == 'ip' ? $aRow->ip : $aRow->url) . '" class="delete_log_group" data-redirect="" data-msg-success="Group deleted" data-btn-confirm="Delete group" data-title="Delete this group" data-wait-msg="Deleting. Please wait." data-name="" title="Delete this group"><i class="wf301-icon wf301-trash"></i></div>';
            $output['aaData'][] = $row;
        } // foreach row
