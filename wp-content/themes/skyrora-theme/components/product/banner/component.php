    ...
    <?php if (have_rows('acf_grop_left')) : ?>
        <?php while (have_rows('acf_grop_left')) : the_row(); ?>
            ...
            <?php } else { ?>
                ...
                <div id="bannerVideoId" data-src="<?php echo get_the_post_thumbnail_url(get_sub_field('acf_product_banner_image')['ID'], 'large'); ?>" class="banner__video banner--image">
                    <div class="bv-video-wrap bv-video-wrap-0" style="position: relative; overflow: hidden; z-index: 10;">
                        <video autoplay="true" playsinline muted loop="true" poster="<?php echo get_sub_field('acf_product_banner_image')['url']; ?>" class="bv-video" preload="metadata" style="position: absolute; z-index: 1;">
                            <source src="<?php echo get_the_post_thumbnail_url(get_sub_field('acf_product_banner_image')['ID'], 'large'); ?>" type="video/mp4">
                        </video>
                    </div>
                </div>
            <?php } ?>
